...
export default function SNSCampaignDesigner() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const prompt = `以下は商材「${productText}」に関する地方中小企業の経営診断結果です。設問はすべて5段階評価で自動回答されています。これをもとに、各フレームワークごとに要点を簡潔にまとめた分析（PEST分析、4C分析、SWOT分析、STP分析、4P分析）とSNSキャンペーンの提案を行ってください。各見出しをつけて出力してください。\n\n診断回答:\n${JSON.stringify(answers, null, 2)}`;
